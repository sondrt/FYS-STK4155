...
def simulation(func,n,e,s):
    ...
    savefig("supervised_%s_%s_%s_%s.pdf" % (activation_function.__name__,nodes,epoch,str(step).replace(".","")) ,bbox_inches="tight")
    clf()
...
if __name__ == '__main__':
    ...
    e_ = [100,1000,10000]
    s_ = [0.1,0.01,0.001]
    outfile = open("supervised.txt","w")
    outfile.write("func    node    epoch   step    MSE     R2")
    for func in func_:
        for n in n_:
            for e in e_:
                for s in s_:
                    ...
                    outfile.write("%s %s %s %s %.3f %.7f\n" % (activation_function.__name__,n,e,str(s).replace(".",""),mse,r2))
    outfile.close()
